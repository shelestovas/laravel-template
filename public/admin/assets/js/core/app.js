...
$(function() {
    ...
    if($('.generate-label-absolute').length > 0) {

        var $inputLabelAbsolute = $('.label-indicator-absolute input');
        var $outputLabelAbsolute = $('.label-indicator-absolute > span');

        var feedback = [
            {color: '#D55757', text: 'Слабый', textColor: '#fff'},
            {color: '#EB7F5E', text: 'Нормальный', textColor: '#fff'},
            {color: '#3BA4CE', text: 'Хороший', textColor: '#fff'},
            {color: '#40B381', text: 'Сильный', textColor: '#fff'}
        ];
    ...
    $(document).on('click', '.remove-item-btn', function(){
        if(confirm('Вы действительно хотите удалить эту запись из базы?')) {
            return true;
        }
        return false;
    });

    $(document).on('submit', '.datatable-remove-row', function(){
        if(confirm('Вы действительно хотите удалить эту запись из базы?')) {
            var _this = $(this),
                _table = _this.closest('table');
            blockui(_this);

            //ajaxRemoveDatatableRow(action_url, data, unblock, _this, callback);
            $.ajax({
...
function renderError(data) {
    ...
    if(data.status == 502 || data.status == 504) {
        return new PNotify({
            title: 'Ошибка!',
            text: 'Сервер не успел обработать запрос. Попробуйте еще раз.',
            addclass: 'bg-danger'
        });
    } else if(data.responseJSON.result == 'system_error') {
        return new PNotify({
            title: data.responseJSON.notify_title,
            text: data.responseJSON.msg,
            addclass: data.responseJSON.notify_class
        });
    } else {
        error_text = '';
        i = 0;
        ...
        $.each(data.responseJSON, function(index, value) {
            /*if($('*[name="' + index + '"]').length > 0) {
             if ($('*[name="' + index + '"]').attr('type') == ('radio' || 'checkbox'))
             valid_check(index, value);

             if ($('*[name="' + index + '"]').attr('type') == 'text')
             valid_text(index, value);

             if ($('*[name="' + index + '"]')[0].nodeName.toLowerCase() == 'textarea')
             valid_text(index, value);

             if ($('*[name="' + index + '"]')[0].nodeName.toLowerCase() == 'select')
             valid_select(index, value);
             }*/
            error = value + '<br />';
            if(error != old_error) {
                i++;
                error_text += i + '. ' + value + '<br />';
            }
            old_error = value + '<br />';
        });
        new PNotify({
